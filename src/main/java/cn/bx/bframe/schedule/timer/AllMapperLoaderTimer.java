package cn.bx.bframe.schedule.timer;
 ...
 * @author bcm
 */
public class AllMapperLoaderTimer implements  ApplicationContextAware {
	...
	public void init(){
		System.out.println("================MyBatis Mapper Loader init===============");
		// 获取xml所在包
		try {
			...
			e.printStackTrace();
			System.out.println("AllMapperLoaderTimer初始化错误");
		} 
	}
	public void scanMapper(){
		if(basePackage == null || scanner == null){
			init();
		}	
		try {
			...
			e.printStackTrace();
			System.out.println("Mapper文件加载错误!");
		}
	}
	...
	class Scanner {
		...
		public Scanner() {
			basePackages = StringUtils.tokenizeToStringArray(AllMapperLoaderTimer.this.basePackage,
				...
				if (resources != null) {
					for (int i = 0; i < resources.length; i++) {
						...
						if (!multi_key.equals(value)) {
							System.out.println("Mapper文件 【"+name+"】重新加载");
							isChanged = true;
